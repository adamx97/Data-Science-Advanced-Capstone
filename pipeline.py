# from: https://github.com/Eligijus112/twitter-genuine-tweets/blob/master/pipeline.py
import numpy as np
...
class Pipeline:
    ...
    """
    def __init__( self, X_train: list, Y_train: list, embed_path: str, embed_dim: int, stop_words=[], X_test=[], Y_test=[],
        max_len=None, epochs=3, batch_size=256):
        ...
        self.TextToTensor_instance = TextToTensor(
            tokenizer=tokenizer, 
            max_len=max_len
            )
        X_train = self.TextToTensor_instance.string_to_tensor(X_train)

        if len(X_test) > 0: 
            X_test = [clean_text(text) for text in X_test]
            X_test =self.TextToTensor_instance.string_to_tensor(X_test)
        # Creating the model
        rnn = RnnModel(
